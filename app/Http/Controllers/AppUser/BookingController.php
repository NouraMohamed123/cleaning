...
class BookingController extends Controller
{
    ...
    public function bookMultipleServices(Request $request)
    {
        ...
        $convertedDate = $requestedDate->format('Y-m-d');
        $startTime = Carbon::createFromFormat('h:i A', $request->time)->format('H:i:s');
        $bookings = [];
        $total_price_all;
    //     /////////////////////////
        ...
        $existingBookings = Booking::whereIn('service_id', $request->service_ids)
            ->where('date', $convertedDate)
            ->get();
        ...
        foreach ($request->service_ids as $serviceId) {
            // Repeat similar logic for each service
            $service = Service::findOrFail($serviceId);
            $totalPrice = $request->meter * $service->price; // Assuming meter applies to all in the same way

            // Check for existing bookings logic here...

            // Create the booking for each service
            $booking = Booking::create([
                'user_id'     => $user->id,
            ...
            $total_price_all += $request->totalPrice;
            $bookings[] = $booking;
        }
        ...
        // Return response
        return response()->json(['message' => 'Booking successful for all selected services', 'bookings' => $bookings], 201);
    }
